...
export default function MinutesForm() {
  ...
  console.log("Watching editor values", watch(), "data url", applications);
  ...
  return (
    <FormProvider {...methods}>
      ...
      >
        ...
        <TextEditor disabled={false} content={content} name="minutes" />
      </form>
    </FormProvider>
  );
}
...
var contents = ({
  ...
  applications: { total, recommended, approved, deferred },
    ...
    <h1 style="font-size:20px;text-align:center;padding:20px 0;font-weight:bold;">
      ...
      OF (${meeting?.office?.initials.toUpperCase()}) HELD ON THE ${dayjs(
  meeting?.date
)
  .format("D MMMM, YYYY")
  .toUpperCase()} 
    ...
    <p>
      A total of (${total}) development applications were received for the session. Out of this total applications, ${
  recommended - deferred?.length
} were recommended by the Technical Sub-Committee(TSC) 
      for approval by the Spatial Planning Committee(SPC). ${panel === "spc" ? `${approved} of the applications was approved by the SPC.` : ""} ${
        deferred?.length
      } application${
  deferred?.length > 1 ? "s" : ""
} was deferred. Details of the deferred  application${
  deferred?.length > 1 ? "s" : ""
} was as follows:
